...
def eval_mesh(test_mesh, res_dir, ref_dir, reffile_prefix='mesh', resfile_prefix='pred'):
        ...
        center_s = 0.5*(landmark_s[4]+landmark_s[5]) + 0.3*(0.5*(landmark_s[0]+landmark_s[3])-0.5*(landmark_s[4]+landmark_s[5]))
        center_t = 0.5*(landmark_t[4]+landmark_t[5]) + 0.3*(0.5*(landmark_t[0]+landmark_t[3])-0.5*(landmark_t[4]+landmark_t[5]))
        nose_dist_s = np.sqrt(np.sum(np.square(0.5*(landmark_s[0]+landmark_s[3])-0.5*(landmark_s[4]+landmark_s[5]))))
        eye_dist_s = np.sqrt(np.sum(np.square(landmark_s[0]-landmark_s[3])))
        r_s = 1.2*(nose_dist_s+eye_dist_s)/2
        nose_dist_t = np.sqrt(np.sum(np.square(0.5*(landmark_t[0]+landmark_t[3])-0.5*(landmark_t[4]+landmark_t[5]))))
        eye_dist_t = np.sqrt(np.sum(np.square(landmark_t[0]-landmark_t[3])))
        ...
        reconstruct_file = open('./view_BP4D/'+test_mesh+'rec.obj','w')
        gt_file = open('./view_BP4D/'+test_mesh+'gt.obj','w')
...
def eval_meshes(test_list, res_dir, ref_dir):
    ...
    txtfile = open("./BP4D/test/result_BP4D_f.txt",'w')
    #txtfile_s = open('./BP4D/test/stable_info.txt','w')
